...
export default function DeviceTransactionModal({ transaction, onClose, devices }: DeviceTransactionModalProps) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (isEditing) {
        await deviceTransactionsApi.update(transaction.id, transactionData);
        toast({ title: "Transaction updated successfully" });
      } else {
        await deviceTransactionsApi.create(transactionData);
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? "Edit Device Transaction" : "Add Device Transaction"}
          </DialogTitle>
        </DialogHeader>
